...
func (r NetSIPResolver) ResolveSIPServers(ctx context.Context, network, uri string) ([]string, error) {
	...
	resolver := r.Resolver
	if resolver == nil {
		resolver = DNSResolverForServersWithDial(r.DNSServers, r.Timeout, r.DialContext)
	}
	service := "sip"
	if endpoint.Secure {
		service = "sips"
...
func DNSResolverForServers(servers []string, timeout time.Duration) *net.Resolver {
	return DNSResolverForServersWithDial(servers, timeout, nil)
}

// DNSResolverForServersWithDial resolves only through the supplied DNS
// servers and optional network dialer. The dialer lets callers keep DNS on
// the same userspace data plane as SIP.
func DNSResolverForServersWithDial(servers []string, timeout time.Duration, dialContext DialContextFunc) *net.Resolver {
	addrs := normalizeDNSServerAddrs(servers)
	if len(addrs) == 0 && dialContext == nil {
