...
func (backend *Backend) CallReceipt(_ context.Context, input vowifiipc.CallReceiptRequest) (vowifiipc.CallReceipt, error) {
	...
	matches := active != nil && active.request.CallID == input.CallID && active.request.OperationID == input.OperationID &&
		callMediaSessionID(active.request.MediaSessionID, active.request.CallID) == input.SessionID
	if !found {
		if matches {
			if active.terminationConfirmed {
				if err := backend.persistCallTerminalLocked(active, callTerminalSource(active, "confirmed_end")); err != nil {
					backend.mu.Unlock()
					return result, err
				}
				result, found, err = backend.operations.LookupCallReceipt(backend.messageScope, input)
				...
				if !found {
					backend.mu.Unlock()
					return result, errors.New("saved call terminal receipt was not found")
				}
			} else {
				backend.mu.Unlock()
	...
	if matches && active.terminationConfirmed {
		call := active.call
		backend.mu.Unlock()
		var closeErr error
		...
		backend.mu.Lock()
		if backend.activeCall == active && active.terminationConfirmed {
			if closeErr == nil {
