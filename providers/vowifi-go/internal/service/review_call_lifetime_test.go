...
func TestReviewFailedRuntimeStopRestoresCallGuard(t *testing.T) {
	...
	if _, err := backend.Stop(t.Context(), vowifiipc.LifecycleRequest{OperationID: "stop"}); operationCode(err) != "call_end_failed" {
	...
	backend.mu.Unlock()
	if !guarded {
		t.Fatal("failed Stop left the retained paid call without an active safety guard")
	}
	if runtime.closes.Load() != 0 {
		t.Fatal("runtime closed before call termination was confirmed")
	}
	session.setConnected(false, time.Now().Add(-time.Minute))
	reviewAwait(t, func() bool { return session.ended.Load() })
	if call.ends.Load() != 2 {
		t.Fatalf("BYE attempts=%d, want exactly one guarded retry", call.ends.Load())
	}
}
...
func (s reviewFailedCallCommit) Complete(generation, id string, result vowifiipc.OperationResult) error {
	if id == "dial" {
		return errors.New("injected call receipt failure")
	}
...
func TestReviewFailedCallCommitRetainsUnconfirmedCall(t *testing.T) {
	...
	if _, err := backend.StartCall(t.Context(), vowifiipc.StartCallRequest{OperationID: "dial", CallID: "call-1", Callee: "+100", MediaBufferMS: 500}); err == nil {
		t.Fatal("call receipt failure was hidden")
	}
	backend.mu.Lock()
	active := backend.activeCall
	...
	backend.mu.Unlock()
	if !retained {
		t.Fatal("failed receipt and failed BYE discarded the live call handle")
	}
	reviewAwait(t, func() bool { backend.mu.Lock(); defer backend.mu.Unlock(); return backend.activeCall == nil })
	if call.ends.Load() != 2 {
		t.Fatalf("cleanup attempts=%d, want 2", call.ends.Load())
	}
}
...
func (r *reviewVoiceRuntime) StartMediaCall(_ context.Context, request vowifiipc.StartCallRequest) (VoiceCall, error) {
	return r.calls[request.CallID], nil
}

type reviewMediaDirectory map[string]*fakeMediaSession

func (d reviewMediaDirectory) Lookup(id string) (BrowserMediaSession, bool) {
	s, ok := d[id]
	return s, ok
}
...
func (c *reviewBlockingCall) End(ctx context.Context) (voicehost.DialogInfoResult, error) {
	...
	select {
	case <-c.release:
		return c.fakeVoiceCall.End(ctx)
	case <-ctx.Done():
		return voicehost.DialogInfoResult{}, ctx.Err()
	}
}
...
func TestReviewLateHangupCannotClearReplacementCall(t *testing.T) {
	...
	if _, err := backend.StartCall(t.Context(), vowifiipc.StartCallRequest{OperationID: "dial-1", CallID: "call-1", Callee: "+100", MediaBufferMS: 500}); err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		_, err := backend.EndCall(ctx, vowifiipc.EndCallRequest{OperationID: "end-1", CallID: "call-1", ReasonCode: "user_hangup"})
		ended <- err
	}()
	select {
	case <-old.entered:
	case <-ctx.Done():
		t.Fatal("BYE did not start")
	}
	...
	if _, err := backend.StartCall(t.Context(), vowifiipc.StartCallRequest{OperationID: "dial-2", CallID: "call-2", Callee: "+200", MediaBufferMS: 500}); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
		_, _ = backend.EndCall(context.Background(), vowifiipc.EndCallRequest{OperationID: "cleanup-2", CallID: "call-2", ReasonCode: "test_cleanup"})
		close(next.remote)
	})
	release()
	...
	backend.mu.Unlock()
	if !retained || nextSession.ended.Load() {
		t.Fatal("late completion of call-1 removed ownership of call-2")
	}
}
...
func reviewAwait(t *testing.T, ready func() bool) {
	...
	for !ready() {
		select {
		case <-ctx.Done():
			t.Fatal("timed out waiting for the exact call transition")
		case <-ticker.C:
		}
	}
}
