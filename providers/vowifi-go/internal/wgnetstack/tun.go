...
// Package wgnetstack is the WireGuard userspace netstack with one narrow
// extension: TCP dialing from a caller-selected local address and port.
//
// Keep this file aligned with golang.zx2c4.com/wireguard/tun/netstack. The
// extension can be removed when upstream exports an equivalent method.
package wgnetstack
...
// DialContextTCPAddrPortWithBind connects to raddr from the caller-selected
// local tunnel address and port. IMS Security-Agree requires this for the
// protected SIP connection negotiated through port-c/port-s.
...
func (net *Net) ListenTCPAddrPort(addr netip.AddrPort) (*gonet.TCPListener, error) {
	fa, pn := convertToFullAddr(addr)
	return listenTCPWithBindReuse(net.stack, fa, pn)
}
