...
control_artifact_source_head: fb4bce530b460a9b7e34b14368c6c196c0031816
...
phase: E4_CONTROL_DEPLOYED_ENGINE_UNTOUCHED_USIM_MAINTENANCE_FENCE_RECOVERY_REVIEW
deployment_cursor: 精确续接5088c4ea…经独立6负控复审并执行exit0；Control/source已fb4，8275e7855a395e5bc9f2dfb203abe46b32faff9bd8f253d3988f862e532bece8 canonical running/imagef8d47…/restart0，旧c96 stopped/no-restart保留。core成功phase control_verified_engine_replacement_prepared，failed_phase字段仅旧metadata事件，禁止重复execute/continuation。原Engine wrapper tx engine-replace-1787723466-02b410ca360f实际执行失败：line1 requires manual recovery: P-CSCF admission is busy；Engine1仍原7b3eff/3c996/PID3561562/run30af/restart0，未stop/create，maintenance/receipt/pcscfrebind均无；global/line manual_required/defaultpromotion prepared。根因是begin_engine_maintenance调用ordinary paid acquire，它也拒绝USIMfence，并非有据的flock争用。USIM仍同run authseq3/pcsc_card_reset；旧service-unavailable专用恢复入口和pristine abort均不适用，禁止改cause假装适用、清fence或盲重跑。正在预审仅本次exact maintenance续接：保留globalEX/event/paid0/channel0/fullsource/合法USIMfence身份，仅原begin写合法prepared marker，然后原wrapper继续；永久maintenance边界缺陷列D2，不假称已产品修复。plan d34e1e3d…9003f9；record1345/stage/finalize_control.py SHA1f6bc4fa…尚未执行。旧源码/SQLite已离机同步private mdd-e4-release-incoming/production-cutover-record，DBSHA43cd2355401fd93d12fe9c81cdabd557cc2185c674f73eb234f879b2335325c9。所有新失败/raw与续接证据在private mdd-e4-release-incoming及mdd-e4-control-pre-review-fb4bce5；先核该游标，不能重做E3/runner/媒体设计。
...
E. 用户确认媒体入口 IP：人工确认仍在生产页面，禁止再误报已删除；它只能在E4随旧付费通话路径一并删除。最终架构已经修订并
   ...
   2026-08-25 强制上游研究又发现官方 Asterisk 自 20.16 起已提供 `chan_websocket`：每通独立媒体 WSS、
   slin 自动 20ms reframing/timing、JSON control、XON/XOFF/STATUS，官方示例直接覆盖 WebSocket→Echo。
   最新官方 20 LTS 为 20.20.1，但 sysmocom IMS/AKA 分支和可借鉴的 pagecat/vowifi_gateway 均仍停在
   20.7；把 67 个 IMS 提交整体重基到 20.20 在第 4 个鉴权提交即出现多文件冲突，当前批排除。隔离研究证明
   把官方 websocket client/auth prerequisites、chan_websocket 与适用修复最小回移到 20.7 可与现有 IMS、
   AMR、SMS 和 MDD admission 补丁共同编译。A/B共存没有模块冲突，但为临时比较保留A需要另一套实时传输
   和大幅测试矩阵，用户授权的复杂度边界已触发，因此只采用官方模块回移；AudioSocket仍由Asterisk编译，
   但不是MDD可选backend。
   B 方案来电最终应答线性点已经修正：exact AMI Bridge 先把 IMS 与 winner 加入同一两腿 bridge，且实证
   此动作不会提前 Answer IMS；两条 BridgeEnter 闭合后，Engine-local `MddAnswerBridged` 将请求排入 IMS
   自己的 bridge-channel serializer，callback 重新核验 armed marker、exact run/owner/call、两腿 bridge、状态
   和 timeout，原子消费 marker 后唯一调用 20.7 现有 `ast_raw_answer()`。禁止误用表示 driver→core 报告的
   `AST_CONTROL_ANSWER`。任何竞态/响应丢失/结果 unknown 都精确挂断双腿且不开放第二 winner。
...
| `CONTROL-CONFIG-CACHE-CPU-20260825` | 开发空闲期 Control 持续约67%–76% CPU | `已修复、已复审、已部署、生产审计PASS` | py-spy 实证多个轮询器反复解析同一 YAML；`config.load()` 改为按同一打开fd的realpath/dev/ino/size/mtime_ns缓存合并解析结果并deepcopy，save失效。部署后GIL采样37个中YAML/parser=0，连续CPU约2.62%–7.79%，Control image=`sha256:909c29ff...`，记录 `/opt/mdd-gateway/data/deploy-records/codex-20260825T224453+0800-config-cache-cpu`。本项目开发阶段所有非peak持续高CPU继续按缺陷处理，不得以“空闲负载可接受”关闭。 |
| `BROWSER-MEDIA-AUTO-PRE` | 统一同源 WS/WSS 浏览器媒体层 | `PASS` | 放弃把Host/interface/IP猜测当最终架构；外部只复用8443，最终选择原生per-call chan_websocket。分E1 Echo、E2呼出、E3呼入、E4删除旧确认，禁止跨阶段误报。 |
| `BROWSER-MEDIA-E1-IMP` | 旧 Browser→Control→persistent relay→AudioSocket Echo | `SUPERSEDED，未部署` | 旧实现虽复审PASS，但强制上游研究发现官方chan_websocket后没有构建生产镜像或部署；代码历史保留证据，当前差异删除media_relay.py/9073/multiplex和相应测试，不将过渡实现带入产品。 |
| `BROWSER-MEDIA-E2-E3-MULTICLIENT-PRE` | 多来源占用、抢接、长时 WSS PCM 与恢复 | `NEEDS_CHANGES×3 → B方案设计 PASS P0/P1/P2=0；用户已选择B` | 最终方案为durable LineCallSession/tombstone、startup unknown仍占用、新旧入口同代互斥、winner用户手势+exact media后first CAS、snapshot+单调revision、ignore-here/全局拒接分离、server epoch+双向64-bit seq、每通独立browser/Asterisk WSS、browser最多一次rebind而Asterisk WSS断直接结束。E2同一WebSocket腿完成`n→MEDIA_START→JSON ANSWER→Echo→timeout→owner marker→Redirect→local paid gate→Dial`；E3先BridgeEnter×2/IMS未Up，再由serializer内`MddAnswerBridged`唯一`ast_raw_answer(IMS)`；unknown双腿挂断。 |
...
| `BROWSER-MEDIA-B-E3-STORE-CAS-IMP1` | 多端入呼durable状态/CAS/维护与删除门禁 | `实施后复审PASS P0/P1/P2=0；已提交、未部署` | commit=`e4efb4093c02535dc015da570996b272f30c0bf5`。calls新增固定browser state/revision/owner三元组；旧open null在同一`BEGIN IMMEDIATE` migration中变unknown，partial schema Host直接UNKNOWN。仅new exact incoming插入ringing0，duplicate call_in在所有状态只补展示字段。SQLite exact CAS绑定instance/id/full source/run/open/state/revision/owner；call_result与旧exact finalize均browser-aware terminal+revision且重复不增；absence terminal也要求exact expected。claiming/attach/answer/active/ending/unknown计入既有`open_call_leases`三键，损坏组合fail-closed。call delete/clear/soft-delete事务内guard；soft/hard instance delete由server-owned shield task从begin fence持有到finally end，record_call_start同事务检查fence/soft-deleted；lifespan在关闭runtime/AMI/Docker前等待全部delete task，HTTP取消和to_thread阻塞不会提前清fence。相关`184 passed, 13 subtests`、pycompile/diff PASS；最终复审PASS。仍不含prepare/AMI/UI，不部署。 |
| `BROWSER-MEDIA-B-E3-INBOUND-PREPARE-IMP1` | 精确呼入prepare、owner分类与最多3 claimant的无资费warmup | `三轮实施后复审最终PASS P0/P1/P2=0；已提交、未部署` | commit=`d5526c76ad8968ae6f69ccd98b24e92a29126332`。prepare仅接受exact instance/call/source/run的durable ringing revision；先原子预留每call最多3、全局16的claimant再做AMI，故容量满时owner snapshot为0。完整快照把pristine与Up/bridged/partial owner区分；AMI unavailable/incomplete/unreadable为unknown，只拒绝重试且零Store变更/零hangup，只有权威unsafe才ringing→ending并exact hangup。session冻结backend revision，WSS claim后和唯一Originate前均exact复核，旧ticket在runtime/AMI前关闭。固定`browser-media-inbound-warmup`仅admission+10秒Echo，无GROUP/Dial/Redirect/Answer。reservation和正式claimant使用同锁两阶段TTL：commit/expiry只能一方获胜，成功从线性化点得到完整30秒，过期先赢则409且释放。直接影响门`296 passed, 43 subtests`；三轮复审先发现并闭合误挂unknown、旧revision、AMI探测风暴和死ticket竞态，最终PASS。全量测试的26个失败在干净HEAD同组重现，未修改其admission路径长度、旧maintenance fixture、sing-box旧断言或IMS旧断言。生产仍为E2，未执行来电/拨号/短信/APDU。 |
| `BROWSER-MEDIA-B-E3-OWNER-TASK-IMP1` | 单调inbound phase与server-owned answer生命周期 | `实施后复审PASS P0/P1/P2=0；已提交、未部署` | commit=`c27c3d62bf0f5eb7ff242ed34b1f0cddfcb70308`。registry在同一`phase_lock→registry lock`临界区创建/登记唯一owner task并暂停claimant TTL；同一task贯穿claim/attach/answer/lease/cleanup，不由WSS finally取消。异常/取消/create_task失败均受控记录、abort、本地close、释放owner map/容量；ending/terminal后迟到task无法推进。相关`137 passed, 19 subtests`；复审发现并修复infinite-TTL泄漏、wrapper coroutine泄漏与静默异常，最终PASS。 |
| `BROWSER-MEDIA-B-E3-AMI-PRIMITIVES-IMP1` | exact双腿freeze/bind/Redirect/Answer/revoke/Hangup/absence原语 | `实施后复审PASS P0/P1/P2=0；已提交、未部署` | commit=`4d8c3337fe1e2956ca2e7618ac3b0ff87ef0466f`。freeze与bind分离，部分写前server已持有IMS+winner exact pair；winner timeout与所有非武装身份先Set/Get，ATTACH倒数第二、ARMED最后。Redirect/Answer固定context/action且primitive不重试；unknown后fresh no-reconnect session分类。revoke独立连接，winner/IMS各自fresh单腿Hangup，第三fresh只读连接连续双snapshot absence；channel name复用不误挂。pair snapshot在owner Getvar后再次完整snapshot，仅返回末次exact映射/两成员bridge/state。相关`166 passed, 41 subtests`；最终复审PASS。 |
| `BROWSER-MEDIA-B-E3-BACKEND-IMP1` | first-winner CAS、唯一Attach/Answer、active lease、terminal/restart/decline闭环 | `多轮实施后复审PASS P0/P1=0；性能P2另行PASS；已提交、未部署` | commit=`573ce4810869c4f07463b2c58fd4d0504b95f766`。Store `BEGIN IMMEDIATE`同事务确保同iid只有一个paid owner，跨进程双连接交错仅一winner；loser本地answered_elsewhere且abort可打断recovery-lock等待。Redirect/Answer在Store unknown落盘后各最多提交一次；最后一次async generation核验后使用无await同步submission guard复核abort/phase/exact Store revision+owner+fresh媒体，用户decline/call_result插入phase broadcast时action=0。Answer后fresh post-proof要求exact两腿bridge、双方Up、ARM0、RESULT answered才active。每通一次登录、never-reconnect AMI每3秒续winner→IMS timeout10并复核完整owner/bridge；WSS/媒体/owner/generation任一unknown即ending+revoke+两条fresh Hangup+双absence。call_result terminal优先并关闭exact call全部claimant；shutdown先abort/close并45秒bounded join。exact decline先CAS ending并abort owner再Hangup。startup global→per-line pending gate；old run必须完整枚举所有managed/retained/renamed Engine，paused/restarting/unknown保持pending，只有exited/dead可忽略；same-run只允许exactly one pristine ringing，其余cleanup或持续pending。inbound DTMF fresh OneShot且要求exact active pair/owner/bridge。最终宽集`506 passed, 56 subtests`，仅第三方Starlette deprecation warning；全量旧26失败已在干净HEAD同组重现。生产仍E2，未真实来电/拨号/短信/APDU。 |
