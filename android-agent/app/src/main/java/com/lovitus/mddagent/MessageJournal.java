...
final class MessageJournal {
    ...
    static void observe(JSONObject config,String scope,JSONArray events)throws JSONException {
        ...
        for(int i=0;i<records.length();i++){
            JSONObject row=records.getJSONObject(i);
            if(!scope.equals(row.optString("scope"))||resolved(row))continue;
    ...
    static JSONObject receipt(JSONObject row)throws JSONException{
        if(resolved(row)||row.optString("body").isEmpty()||row.optString("recipient").isEmpty()||row.optString("card_id").isEmpty()||row.optString("message_id").isEmpty()||row.optString("operation_id").isEmpty())throw new IllegalStateException("缺少原提交内容；请查原网关历史，不能用当前输入重试");
        JSONObject payload=Json.obj("operation_id",row.getString("operation_id"),"message_id",row.getString("message_id"),"recipient",row.getString("recipient"),"body",row.getString("body"),"expected_card_id",row.getString("card_id"));
        if(row.getString("transport").equals("cellular"))payload.put("reconcile_only",true);else if(!row.getString("transport").equals("vowifi"))throw new IllegalStateException("Unknown SMS transport");return payload;
    }
    static void adoptConfirmedTrustChange(JSONObject config,String origin,String pin,String authenticatedUser)throws JSONException{
        ...
        for(String priorPin:trusted){if(priorPin.equals(pin))continue;String oldScope=GatewayApi.scope(origin,priorPin,authenticatedUser);
            ...
            MessageSync.adoptTrustChange(config,oldScope,newScope);
        }
    }
}
