...
final class Link implements AutoCloseable {
    interface Events {void message(JSONObject o);void state(int label,boolean connected);}
    ...
    private void open(){
        ...
        opened=lastMessage=SystemClock.elapsedRealtime();acknowledged=false;sequence=0;revision="";events.state(R.string.link_connecting,false);
        socket=api.http.newWebSocket(request.build(),new WebSocketListener(){
            ...
            public void onMessage(WebSocket ws,String text){post(()->{if(!current(mine,ws))return;try{
                ...
                if(!agentID.isEmpty()&&!acknowledged){if(!o.optString("kind").equals("hello_ack"))throw new IllegalArgumentException();acknowledged=true;events.state(R.string.link_reader_online,true);}
                else if(agentID.isEmpty()){String type=o.optString("type");if(!type.equals("mobile.snapshot")&&!type.equals("mobile.heartbeat"))throw new IllegalArgumentException();if(o.optInt("schema_version")!=1)throw new IllegalArgumentException();if(!acknowledged&& !type.equals("mobile.snapshot"))throw new IllegalArgumentException();acknowledged=true;events.state(R.string.link_online,true);}
                if(lastMessage-opened>30000)retry.healthy();events.message(o);
            }catch(Exception invalid){terminal=true;failed(mine,R.string.link_schema,false);}});}
            public void onClosing(WebSocket ws,int code,String reason){ws.close(code,null);}
            public void onClosed(WebSocket ws,int code,String reason){post(()->{if(!current(mine,ws))return;if(code==4401||code==1008)terminal=true;failed(mine,terminal?R.string.link_auth_required:R.string.link_interrupted,!terminal);});}
            public void onFailure(WebSocket ws,Throwable error,Response response){post(()->{if(!current(mine,ws))return;int code=response==null?0:response.code();boolean identityFailure=identityFailure(error);terminal=code==401||code==403||code==404||identityFailure;android.util.Log.w("MddLink",(agentID.isEmpty()?"observer":"reader")+" transport failure: "+error.getClass().getName()+"; identity_rejected="+identityFailure);failed(mine,code==404?R.string.link_upgrade:terminal?R.string.link_auth_tls:R.string.link_wait_network,!terminal);});}
        });
    ...
    void networkChanged(){post(()->{
        ...
        if(deadline!=null)deadline.cancel(false);
        events.state(R.string.link_network_changed,false);
