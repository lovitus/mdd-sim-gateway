...
public class LinkTest {
    ...
    @Test public void missingMobileEndpointRetriesUntilCoreRouteAppears()throws Exception{
        ScheduledExecutorService loop=Executors.newSingleThreadScheduledExecutor();
        HeldCertificate cert=new HeldCertificate.Builder().addSubjectAlternativeName("localhost").build();
        try(MockWebServer server=new MockWebServer()){
            server.useHttps(new HandshakeCertificates.Builder().heldCertificate(cert).build().sslSocketFactory(),false);
            server.enqueue(new MockResponse().setResponseCode(404));
            CountDownLatch online=new CountDownLatch(1),upgrade=new CountDownLatch(1);AtomicReference<WebSocket> peer=new AtomicReference<>();
            server.enqueue(new MockResponse().withWebSocketUpgrade(new WebSocketListener(){
                @Override public void onOpen(WebSocket socket,Response response){peer.set(socket);socket.send(Json.obj("type","mobile.snapshot","schema_version",1,"sequence",1,"data",Json.obj("lines",new org.json.JSONArray())).toString());}
            }));server.start();
            GatewayApi api=new GatewayApi(new Endpoint(server.url("/").toString(),Json.sha(cert.certificate().getEncoded())),"session","csrf");
            Link link=new Link(api,loop,"/v1/mobile/ws","","","process",new Link.Events(){
                public void message(JSONObject message){}
                public void state(int label,boolean connected){if(label==R.string.link_upgrade)upgrade.countDown();if(connected)online.countDown();}
            });
            try{
                link.connect();assertTrue("A missing Core route must be surfaced",upgrade.await(5,TimeUnit.SECONDS));
                assertTrue("The same app process must recover when the Core route is upgraded",online.await(10,TimeUnit.SECONDS));
                RecordedRequest first=server.takeRequest(2,TimeUnit.SECONDS),second=server.takeRequest(2,TimeUnit.SECONDS);
                assertNotNull(first);assertNotNull(second);assertEquals("/v1/mobile/ws",first.getPath());assertEquals("/v1/mobile/ws",second.getPath());
                assertEquals("GET",first.getMethod());assertEquals("GET",second.getMethod());
                assertNull("Successful recovery must not leave a rapid retry loop",server.takeRequest(800,TimeUnit.MILLISECONDS));
            }finally{link.close();loop.submit(()->{}).get(2,TimeUnit.SECONDS);api.close();}
        }finally{loop.shutdownNow();}
    }
