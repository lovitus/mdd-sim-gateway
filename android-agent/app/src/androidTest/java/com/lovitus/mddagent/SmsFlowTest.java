...
public class SmsFlowTest {
    ...
    @Test public void newAccountSyncDoesNotWaitForOrCommitTheOldResponse()throws Exception{
        run("vowifi",(fixture,scene)->{
            ...
            assertTrue(fixture.oldSyncReturned.await(5,TimeUnit.SECONDS));scene.recreate();await(fixture,()->view(scene,R.id.home_availability_toggle,false)&&syncIdle(scene));assertEquals(fixture.otherScope,fixture.store.load().getString("account_scope"));assertEquals(fixture.stream+":0",MessageSync.state(fixture.store.load(),fixture.scope).getString("cursor"));assertEquals(0,fixture.sends.get());
        });
    }
    ...
    private void run(String mode,Scenario scenario)throws Exception{permission(true);try(SmsGatewayFixture fixture=new SmsGatewayFixture(context(),mode)){
        try(ActivityScenario<MainActivity> scene=ActivityScenario.launch(MainActivity.class)){assertTrue(fixture.connected.await(10,TimeUnit.SECONDS));await(fixture,()->synced(fixture,0)&&view(scene,R.id.home_availability_toggle,false)&&syncIdle(scene));scenario.run(fixture,scene);fixture.check();}
