...
func (r CallReceipt) TerminalOutcome() string {
	if r.Outcome != "" {
		return r.Outcome
	}
	if r.Source == "confirmed_rejected" {
		return "rejected"
	}
	return "ended"
}
...
func (r CallReceipt) Validate() error {
	if r.CallReceiptRequest.Validate() != nil || !validIdentifier(r.LineID) || !validIdentifier(r.ProviderID) || !validIdentifier(r.ProcessGeneration) || r.ConfirmedAt.IsZero() || r.ConfirmedAt.After(time.Now().Add(time.Minute)) || (r.Source != "carrier_bye" && r.Source != "confirmed_end" && r.Source != "confirmed_rejected") {
		return errors.New("invalid call terminal receipt")
	}
	if r.Source == "confirmed_rejected" && r.TerminalOutcome() != "rejected" || r.Source != "confirmed_rejected" && r.TerminalOutcome() != "ended" {
		return errors.New("invalid call terminal receipt")
	}
	return nil
}
