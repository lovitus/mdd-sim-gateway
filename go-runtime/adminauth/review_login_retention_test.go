...
func TestReviewOverloadedLoginsDoNotAllocatePeerHistory(t *testing.T) {
	...
	for i := 0; i < maxLoginDerivations; i++ { manager.loginInFlight[fmt.Sprintf("busy-%d", i)] = true }
	...
	for i := 0; i < 1000; i++ {
		...
		if !errors.As(err, &throttle) { t.Fatalf("overload error=%v", err) }
	}
	if len(manager.failures) != 0 { t.Fatalf("overloaded requests allocated %d peer-history entries", len(manager.failures)) }
}
...
func TestReviewLoginHistoryHasBoundAndExpires(t *testing.T) {
	...
	for i := 0; i < 5000; i++ { _, _ = manager.Login("fanli", "wrong", fmt.Sprintf("peer-%d", i)) }
	if len(manager.failures) > 4096 { t.Fatalf("unbounded peer history: %d", len(manager.failures)) }
	now = now.Add(16*time.Minute)
	_, err := manager.Login("fanli", "wrong", "fresh-peer")
	if !errors.Is(err, ErrInvalidCredentials) { t.Fatalf("expired history prevented a fresh attempt: %v", err) }
	if len(manager.failures) != 1 { t.Fatalf("expired peer histories retained: %d", len(manager.failures)) }
}
