...
function selectView(view){state.view=view;for(const button of document.querySelectorAll("[data-view]"))button.classList.toggle("active",button.dataset.view===view);for(const section of document.querySelectorAll(".view"))section.classList.toggle("hidden",section.id!==`view-${view}`);if(view==="settings"){if(!state.runtime)loadRuntime();loadLineCatalog();loadProviderConfig();loadEgressConfig()}if(view==="diagnostics")loadDiagnostics();if(view==="calls")refreshCallStatuses();if(view==="messages")refreshSelectedCellularMessages();if(view==="esim")loadEUICCs()}
