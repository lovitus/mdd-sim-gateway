...
const state={csrf:"",socket:null,snapshot:null,devicesLoading:false,diagnostics:new Map(),runtime:null,lineCatalog:null,lineCandidates:null,lineCandidatesLoading:false,rawModem:null,rawModemLine:"",rawModemLoading:false,providerConfig:null,egressConfig:null,egressDraft:null,egressApply:null,diagnosticSnapshot:null,egressDiagnosticSnapshot:null,egressDiagnosticResults:new Map(),euiccs:null,euiccLoading:false,euiccDownloads:new Map(),view:viewFromHash(),pendingMessage:null,messageSending:false,currentCall:null,providerStatuses:new Map(),cellularStatuses:new Map(),cellularData:new Map(),callStatusLoading:false,callHistory:[],callHistoryLoading:false,dtmfSending:false,dtmfEcho:""};
...
function openConsole(){loginPanel.classList.add("hidden");selectView(viewFromHash(),false);consolePanel.classList.remove("hidden");el("logout").classList.remove("hidden");restorePendingMessage();connectState();loadRuntime();loadLineCatalog();loadLineCandidates();loadProviderConfig();loadEgressConfig();loadDiagnostics();loadEUICCs();if(state.view==="calls"){refreshCallStatuses();loadCallHistory()}if(state.view==="messages")refreshSelectedCellularMessages()}
...
async function loadRawModemBinding(line){
  ...
  if(!/^\d{4,32}$/.test(line.card_id||"")||!/^\d{14,16}$/.test(line.sim?.imei||"")){state.rawModemLoading=false;renderRawModemUnavailable("该线路没有精确 ICCID 与 IMEI，只能使用适配模式");return}
...
function renderRawModemBinding(){
  ...
  const sourceIDs=[...new Set((view?.candidates||[]).map(candidate=>candidate.source_agent_id))].sort(),importerIDs=[...(view?.importers||[])];
  appendRawModemOptions(source,sourceIDs,binding?.source_agent_id,"当前来源 Agent 未在线或身份已变化");appendRawModemOptions(importer,importerIDs,binding?.importer_agent_id,"当前导入 Agent 未在线");
  mode.value=binding?.enabled?"raw":"adapted";source.value=binding?.source_agent_id||sourceIDs[0]||"";importer.value=binding?.importer_agent_id||importerIDs[0]||"";syncRawModemControls();
...
async function saveRawModemBinding(){
  ...
  const source=el("raw-modem-source").value,importer=el("raw-modem-importer").value;if(raw&&(!source||!importer||source===importer)){showRawModemResult("请选择不同的来源 Agent 与服务端导入 Agent",true);return}
  if(raw&&!confirm(`将当前线路的整个 Modem 从 ${source} 透传到 ${importer}。系统会在实际接管前再次核对 Agent、IMEI、ICCID、SIM/AT、数据隔离和通话空闲；不匹配时不会猜测或切换设备。`))return;
  button.disabled=true;showRawModemResult(raw?"正在重新核对精确设备与卡片后保存整机透传…":"正在保存适配模式；活动通话不会被强制拆除…");
  const expectedEquipment=raw?view.equipment_id:(view.binding?.equipment_id||view.equipment_id),expectedCard=raw?view.card_id:(view.binding?.card_id||view.card_id);
  try{const updated=await jsonRequest(`/v1/lines/${encodeURIComponent(lineID)}/raw-modem`,{method:"PUT",body:JSON.stringify({expected_revision:view.revision,expected_equipment_id:expectedEquipment,expected_card_id:expectedCard,enabled:raw,source_agent_id:raw?source:"",importer_agent_id:raw?importer:""})});if(state.rawModemLine!==lineID)return;state.rawModem=updated;renderRawModemBinding()}
...
function renderMessageLines(lines){
  ...
  const pending=state.pendingMessage,locked=pending?messageRouteValue(pending.mode||"vowifi",pending.line_id):"";let options=[...select.options],preferred=options.find(option=>option.value===(locked||selected));if(locked&&!preferred){preferred=appendMissingLockedRoute(select,locked,`原短信线路 ${pending.line_id} 已不可用 · 保留同一幂等请求，未切换到其他 SIM`);options=[...select.options]}const firstReady=options.find(option=>!option.disabled);select.value=preferred&&(locked||!preferred.disabled)?preferred.value:(firstReady?.value||options[0]?.value||"");syncMessageControls();
}

function messageRouteValue(mode,lineID){return`${mode}:${lineID}`}
function selectedMessageRoute(){const select=el("message-line"),option=select.selectedOptions[0],value=select.value,separator=value.indexOf(":");if(separator<1||option?.disabled)return null;return{mode:value.slice(0,separator),line_id:value.slice(separator+1)}}
function syncMessageControls(){const select=el("message-line"),pending=Boolean(state.pendingMessage),routeReady=Boolean(select.value)&&select.selectedOptions[0]?.disabled!==true,anyReady=[...select.options].some(option=>!option.disabled);select.disabled=pending||!anyReady;el("message-send").disabled=state.messageSending||!routeReady;el("message-refresh").disabled=!routeReady||!select.value.startsWith("cellular:")}
