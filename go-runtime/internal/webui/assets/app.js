...
const state={csrf:"",socket:null,snapshot:null,diagnostics:new Map(),runtime:null,lineCatalog:null,providerConfig:null,diagnosticSnapshot:null,view:"overview",pendingMessage:null,messageSending:false,currentCall:null,providerStatuses:new Map(),callStatusLoading:false};
...
async function beginCall(event){
  ...
  const call={line_id:el("call-line").value,callee,buffer_ms:bufferMS,call_id:callIdentity("browser-call"),start_operation_id:callIdentity("ui-call-start"),end_operation_id:"",lease:null,media:null,phase:"preparing",ending:false,muted:false,direction:"outgoing"};
...
async function submitCallStart(){const call=state.currentCall;if(!call||!["ready","start_unknown"].includes(call.phase))return;el("call-start").disabled=true;const incoming=call.direction==="incoming",path=`/v1/lines/${encodeURIComponent(call.line_id)}/vowifi/${incoming?"calls/incoming/answer":"calls/start"}`,body=incoming?{operation_id:call.start_operation_id,call_id:call.call_id,media_session_id:call.lease?.session_id,media_buffer_ms:call.buffer_ms}:{operation_id:call.start_operation_id,call_id:call.call_id,media_session_id:call.lease?.session_id,callee:call.callee,media_buffer_ms:call.buffer_ms};try{const result=await jsonRequest(path,{method:"POST",body:JSON.stringify(body)});call.phase="active";call.media.markActive();showCallResult(`通话中 · ${call.callee} · ${result.code||"active"}`);await refreshCallStatuses()}
