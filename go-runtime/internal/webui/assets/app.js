...
function openConsole(){loginPanel.classList.add("hidden");selectView(viewFromHash(),false);consolePanel.classList.remove("hidden");el("logout").classList.remove("hidden");restorePendingMessage();connectState();loadRuntime();loadLineCatalog();loadProviderConfig();loadEgressConfig();loadDiagnostics();loadEUICCs();if(state.view==="calls"){refreshCallStatuses();loadCallHistory()}if(state.view==="messages")refreshSelectedCellularMessages()}
