...
func WithReaderProvision(handler http.Handler) Option {
	return func(server *Server) { server.readerProvision = handler }
}
...
func NewServer(replay *events.Replay, now func() time.Time, options ...Option) *Server {
	...
	if server.euiccProfiles != nil {
		...
		server.mux.Handle("POST /v1/euiccs/{eid}/profiles/{iccid}/recovery-codes", server.protect(server.euiccProfiles))
		server.mux.Handle("GET /v1/euiccs/{eid}/deletions", server.protect(server.euiccProfiles))
		server.mux.Handle("POST /v1/euiccs/{eid}/deletions/{operation_id}/recover", server.protect(server.euiccProfiles))
		server.mux.Handle("GET /v1/euiccs/{eid}/notification-archives", server.protect(server.euiccProfiles))
		server.mux.Handle("POST /v1/euiccs/{eid}/notifications/{sequence}/replay", server.protect(server.euiccProfiles))
		server.mux.Handle("GET /v1/euiccs/{eid}/profiles/{iccid}/soft-delete", server.protect(server.euiccProfiles))
		server.mux.Handle("POST /v1/euiccs/{eid}/refresh", server.protect(server.euiccProfiles))
	...
	if server.readerProvision != nil {
		server.mux.Handle("POST /v1/readers/provision", server.protect(server.readerProvision))
	}
