...
func (store *Store) ConfirmRecoveryOutcome(expected RecoveryRecord, at time.Time, source, outcome string) error {
	if at.IsZero() || at.After(time.Now().Add(time.Minute)) ||
		!(expected.Transport == "cellular" && source == "agent_terminal_receipt" && outcome == "ended" ||
			expected.Transport == "vowifi" && source == "provider_terminal_receipt" && (outcome == "ended" || outcome == "rejected")) {
		return ErrRecoveryIdentity
	}
	return store.db.Update(func(tx *bolt.Tx) error {
		bucket := tx.Bucket(recoveryBucket)
