...
func TestReviewMediaRejectsReservationExpiringDuringUpgrade(t *testing.T) {
	...
	if err := broker.Reserve(Reservation{AgentID: "agent-1", ProcessGeneration: "process-1", SessionID: "session-1", MediaToken: mediaTestToken, ExpiresAt: expires}); err != nil { t.Fatal(err) }
