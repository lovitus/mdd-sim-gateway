...
type reviewExpiryWriter struct { http.ResponseWriter; expire func() }
...
func TestReviewDataRejectsReservationExpiringDuringUpgrade(t *testing.T) {
	...
	if err := broker.Reserve(Reservation{AgentID: "agent-1", ProcessGeneration: "process-1", SessionID: "session-1", StreamID: "stream-1", StreamToken: managerTestToken, Network: "tcp", ExpiresAt: expires}); err != nil { t.Fatal(err) }
	...
	if _, payload, err := socket.Read(ctx); err == nil { t.Fatalf("expired bearer admitted: %s", payload) }
}
...
func TestReviewAcquireRechecksDataDeadlineAfterWait(t *testing.T) {
	...
	now := func() time.Time {
		...
		if capture.CompareAndSwap(true, false) { close(entered) }
		return current
	}
	broker, err := NewBroker(agentlink.TokenResolverFunc(func(context.Context, string) (string, error) { return managerTestToken, nil }), now)
	if err != nil { t.Fatal(err) }
	...
	go func() { conn, err := broker.Acquire(ctx, "stream-1"); if conn != nil { _ = conn.Close() }; result <- err }()
	select { case <-entered: case <-ctx.Done(): t.Fatal("Acquire did not check initial reservation") }
	left, right := net.Pipe()
	defer left.Close()
	defer right.Close()
	broker.mu.Lock()
	...
	close(record.ready)
	broker.mu.Unlock()
	if err := <-result; err == nil { t.Fatal("Acquire returned a connection whose lease expired while waiting") }
}
