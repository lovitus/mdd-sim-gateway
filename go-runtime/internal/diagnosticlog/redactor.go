// Package diagnosticlog provides bounded, redacted operator diagnostics.
// The redactor is copied from boa-z/vowifi-go commit 1e9c6e6a and retains its
// shared-placeholder behavior; MDD adds local-path removal for host logs.
...
var (
	...
	sensitiveAssignmentRE       = regexp.MustCompile(`(?i)\b(pin|puk|password|token|secret|api[_-]?key|apdu)\b(\s*[=:]\s*)("[^"]*"|'[^']*'|[^,;\s]+)`)
	...
	localPathRE                 = regexp.MustCompile(`(?i)(?:/(?:[A-Za-z0-9_.-]+/)+[^\s"'<>:;,)]*|[A-Za-z]:\\(?:[^\\\s"'<>:;,)]*\\)+[^\\\s"'<>:;,)]*)`)
	hostnameRE                  = regexp.MustCompile(`(?i)\b(?:[a-z0-9-]+\.)+[a-z]{2,63}\b`)
)
...
func (r *Redactor) RedactString(s string) string {
	...
	out = sensitiveAssignmentRE.ReplaceAllStringFunc(out, redactSensitiveAssignment)
	...
	out = hostnameRE.ReplaceAllStringFunc(out, func(value string) string {
		if strings.Contains(strings.ToLower(value), "redacted-") {
			return value
		}
		return r.placeholder("domain", strings.ToLower(value)) + ".invalid"
	})
	return localPathRE.ReplaceAllString(out, "<redacted-local-path>")
}
...
func redactSensitiveAssignment(param string) string {
	match := sensitiveAssignmentRE.FindStringSubmatch(param)
	if len(match) != 4 {
		return param
	}
	return match[1] + match[2] + quoteLikeParamValue(match[3], "<redacted>")
}
