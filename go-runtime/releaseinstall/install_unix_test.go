...
func TestActivateLockedInstallsUpgradesAndRollsBackWithoutStartingServices(t *testing.T) {
	...
	for link, target := range map[string]string{
		...
		filepath.Join(layout.LibexecDirectory, "mdd-updater"):             filepath.Join(layout.CurrentLink, "mdd-updater"),
		...
		filepath.Join(layout.UnitDirectory, "mdd-updater.service"):        filepath.Join(layout.CurrentLink, "mdd-updater.service"),
		filepath.Join(layout.UnitDirectory, "mdd-updater.path"):           filepath.Join(layout.CurrentLink, "mdd-updater.path"),
...
func testBundle(t *testing.T, output, releaseID, marker string) (string, releasebundle.Manifest) {
	...
	items := []item{
		...
		{"mdd-updater", releasebundle.RoleUpdater, 0o755},
		{"xray", releasebundle.RoleXray, 0o755},
		{"xray-source.tar.gz", releasebundle.RoleXraySource, 0o644},
		{"XRAY-NOTICE.md", releasebundle.RoleXrayNotice, 0o644},
		...
		{"mdd-updater.service", releasebundle.RoleUpdaterUnit, 0o644}, {"mdd-updater.path", releasebundle.RoleUpdaterPath, 0o644},
