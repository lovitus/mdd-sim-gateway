...
func (handler *Handler) recoverCall(w http.ResponseWriter, r *http.Request, line string) {
	...
	terminal := func() {
		...
		write(200, map[string]any{"state": "terminal", "terminal_outcome": outcome, "call_id": record.CallID, "operation_id": record.OperationID, "session_id": record.SessionID, "terminal_confirmed": true, "terminal_at": record.TerminalAt})
	}
	...
	err = handler.providers.UseCurrent(ctx, line, func(provider mediaauth.Provider) error {
		...
		if proofErr == nil {
			...
			outcome := proof.TerminalOutcome()
			if err := store.ConfirmRecoveryOutcome(record, proof.ConfirmedAt, "provider_terminal_receipt", outcome); err != nil {
				return err
			}
			record.TerminalAt, record.TerminalOutcome = proof.ConfirmedAt, outcome
			return nil
		}
