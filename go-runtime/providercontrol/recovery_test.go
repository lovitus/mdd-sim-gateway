...
func TestProviderRecoveryReadsNaturalTerminalAcrossProviderRestart(t *testing.T) {
	store, err := callhistory.Open(filepath.Join(t.TempDir(), "calls.db"), time.Second)
	if err != nil {
		t.Fatal(err)
	}
	...
	if response.Code != 200 || json.Unmarshal(response.Body.Bytes(), &result) != nil || result["terminal_confirmed"] != true || backend.reads != 1 {
	...
	if response = invoke(key); response.Code != 200 || backend.reads != 1 {
...
func TestReviewFixRejectedOutcomePersistsAcrossProviderAndCoreRestart(t *testing.T) {
	...
	if result["outcome"] != "rejected" {
		t.Fatalf("lost rejected outcome: %v", result)
	}
	if len(backend.operations) != 0 {
		t.Fatal("natural terminal lookup sent a mutation")
	}
	...
	if json.Unmarshal(response.Body.Bytes(), &result) != nil || result["outcome"] != "rejected" {
		t.Fatalf("restart lost outcome: %s", response.Body.String())
	}
}
