...
def list_instances(include_deleted: bool = False) -> list:
    insts = list(load()["instances"].values())
    if not include_deleted:
        insts = [i for i in insts if not i.get("soft_deleted")]
    return insts


def list_soft_deleted_instances() -> list:
    return [i for i in load()["instances"].values() if i.get("soft_deleted")]


def soft_delete_instance(iid: str) -> bool:
    with _lock:
        data = load()
        inst = data["instances"].get(str(iid))
        if not inst:
            return False
        inst["soft_deleted"] = True
        inst["soft_deleted_ts"] = int(time.time())
        save(data)
        return True


def restore_instance(iid: str) -> bool:
    with _lock:
        data = load()
        inst = data["instances"].get(str(iid))
        if not inst:
            return False
        inst.pop("soft_deleted", None)
        inst.pop("soft_deleted_ts", None)
        iccid = inst.get("iccid")
        if iccid:
            unsuppress_card(iccid)
        save(data)
        return True
