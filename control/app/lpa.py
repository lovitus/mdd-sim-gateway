...
async def profile_delete(reader_name: str, iccid: str, *, aid: str | None = None) -> Any:
    """Physical profile deletion is permanently disabled by policy to prevent accidental profile destruction."""
    del reader_name, iccid, aid
    raise LpaError(
        "physical profile deletion is disabled",
        detail="Physical eSIM deletion on card is disabled by security policy. Use soft-delete / disable instead.",
        code=-1,
    )
...
async def notification_replay(
    ...
    *,
    ...
    """Explicitly replay a deletion or management notification to SM-DP+/SM-DS with audit tracking."""
    log.info("explicit notification replay requested reader=%s seq=%d autoremove=%s aid=%s",
             reader_name, seq, autoremove, aid)
    return await notification_process(
        reader_name, seq=seq, all_notifications=False, autoremove=autoremove, aid=aid
    )
