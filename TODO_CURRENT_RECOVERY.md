...
更新：2026-08-27。下面媒体容错批次未部署；生产仍为 a8b9faa，勿重放旧批次。
...
所有收费目标只读私有授权文件，结束必须核对实际通话停止；本批尚未执行新的收费测试。
...
- 新bundle已构建，11项校验；本提交冻结源码，镜像尚未构建/部署。下一步
  复用既有增量构建部署流程→三线各一次最多50秒的双向媒体/实际挂断验证。
