...
|---|---|---|---|
| P0 | 登录 bootstrap／改密；reader/card 发现、SIM PIN、卡目录；instance/line CRUD、软删恢复、provision、start/stop/reprovision | Go 只有登录/登出和只读 catalog/line projection；当前挂载的 `SimConfigV1` 没有 reader/PIN/provision 全流程，旧 `SimConfig.jsx` 仍保留 legacy 调用但未作为当前 Devices 子页 | 基础 SIM 管理、线路配置和恢复路径断裂；先迁移或提供明确兼容入口，未完成前不得继续占卡调试 |
| P0 | 系统设置、更新/apply/progress、备份、维护、agent-token、support bundle；通知／出口设置 | `SystemV1` 已接 runtime/preferences 但改密仍调用缺失的 `/api/auth/password`；`NotificationsV1` 覆盖通知主链，`UnifiedPages` 中旧 settings 分支仍存在；更新/备份/维护/token/support 尚无 Go parity | 运维与故障恢复不可用；需先确定单一 API 所有权并逐页接回 |
...
活动 Devices 收敛（尚未部署）：硬件页实际为 `HardwareV1` typed raw-modem 流程；状态页旧 `SmsAdvisory` 的短信刷新
与 modem 软重启尚无 Go Agent action、attachment/session/lease 契约，现已明确置灰并显示“待迁移”，不再调用缺失的
legacy `/api/devices/*` mutation。待后续从旧 MDD/VoCat 复用完整安全契约后再实现，当前不触碰硬件。

P0 reader/card 只读收敛（尚未部署）：`SimConfigV1` 现在直接展示 `/v1/devices` 提供的 reader、CardID、Agent 与 PIN
...
首次 Workflow 的原始失败为 `units_linux_test.go:77`：fixture 返回 Core 和 Guard，但断言把固定顺序中的
index 1（实际 Agent）误当 Guard，再把 index 2 的真实 Guard 误当“响应缺失”。生产实现始终按 unit 名称映射，
修复只让测试建立 `name -> UnitStatus` 并分别核对明确 not-found Guard 与真正缺失的 Agent/Egress/Apply；没有
