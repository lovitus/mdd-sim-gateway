...
原可靠性修复在5a0fb61提交并构建但未部署。实际通话随后发现并追加一个Control call_media修复：
共六个运行文件，仍只用原部署事务，不覆盖或重放716的已完成部署。
...
- HK实拨提交后报`cellular PCM jitter queue overflow`并被保护挂断；该次未接通、已确认清理。
  已复现合法4×640B回调／8帧输入在消费者尚未执行前填满6帧队列。修为有界入队背压，
  保留原收到时间（包括跨块残余）、6帧／0.5秒I/O／0.2秒年龄上限，真正阻塞仍停止并清理。
  修改前9个反例失败；修改后相关5文件202项/21子测试通过，独立96项复审通过；尚未现场部署。
  含全部可靠性改动的最终40文件回归1143项/125子测试通过，16.19秒；部署辅助包14项通过。
...
用户已替换英国测试号码（只读取私有授权文件最新值，旧号不作测试配置）。新号真实接通，
完整TTS已发送、收到4.68秒非静音且非暖机标记下行，同call单次RTP Rx138/Tx193；
接通约5.056秒主动挂断、通道0、通话记录answered且有结束时间。第二次RTP采样与挂断重叠，
未测得两次增量，原脚本ok=false保留；这不是业务拨号失败，也不代表对端听见／浏览器音质已验。

唯一下一步：冻结包含HK队列修复的候选并构建，保留现场后用core1600/原EngineReplacement部署，
再有记录验证HK；在确认清理之前禁止下一通。不要再反复拨英国已通过基本通话的目标来补统计。
