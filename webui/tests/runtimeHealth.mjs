...
const css=fs.readFileSync(new URL('../src/mdd/index.css',import.meta.url),'utf8')
assert.match(css,/\.u-device-option-status\s*\{[^}]*display:flex;[^}]*min-width:0;/)
assert.match(css,/\.u-device-option-status>\[role="status"\][^{]*\{[^}]*white-space:nowrap;[^}]*line-height:18px;/)
