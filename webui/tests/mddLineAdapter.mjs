...
const {rememberProvision,recalledProvision,forgetProvision,provisionTerminal}=await import('../src/mdd/provisionRecovery.js')
const recoveryValues=new Map()
const recoveryStorage={getItem:key=>recoveryValues.get(key)||null,setItem:(key,value)=>recoveryValues.set(key,value),removeItem:key=>recoveryValues.delete(key)}
const pendingProvision={line_id:'line',operation_id:'op',card_id:'card',imsi:'identity',pin:'must-not-store',token:'must-not-store'}
rememberProvision(pendingProvision,recoveryStorage)
assert.equal(recalledProvision('line',recoveryStorage).operation_id,'op')
assert.ok(![...recoveryValues.values()].join('').includes('must-not-store'))
assert.throws(()=>rememberProvision({...pendingProvision,operation_id:'new'},recoveryStorage),/reconcile_required/)
assert.throws(()=>rememberProvision({...pendingProvision,imsi:'changed'},recoveryStorage),/request_changed/)
assert.equal(rememberProvision(pendingProvision,recoveryStorage).imsi,'identity')
assert.throws(()=>rememberProvision(pendingProvision,{getItem:()=>null,setItem:()=>{throw new Error('storage unavailable')}}),/storage unavailable/)
forgetProvision({...pendingProvision,operation_id:'other'},recoveryStorage)
assert.ok(recalledProvision('line',recoveryStorage))
forgetProvision(pendingProvision,recoveryStorage)
assert.equal(recalledProvision('line',recoveryStorage),null)
recoveryValues.set('mdd_provision_recovery_line','{')
assert.throws(()=>recalledProvision('line',recoveryStorage))
recoveryValues.set('mdd_provision_recovery_line',JSON.stringify({...pendingProvision}))
assert.throws(()=>recalledProvision('line',recoveryStorage),/recovery_invalid/)
recoveryValues.clear()
assert.equal(provisionTerminal('in_progress'),false)
assert.equal(provisionTerminal('unknown'),false)
assert.equal(provisionTerminal('reconciled'),true)
const provisionPage=await readFile(new URL('../src/mdd/views/ProvisionActions.jsx',import.meta.url),'utf8')
assert.equal((provisionPage.match(/if \(unresolved\) throw new Error\('provision_reconcile_required'\)/g)||[]).length,2)
assert.ok(provisionPage.includes('if (provisionTerminal(result.state))'),'only terminal outcomes discard the reconciliation identity')
assert.ok(provisionPage.includes('}, [endpointIdentity,candidateRead])'),'candidate read must refresh for a new Agent/card session or explicit retry')
assert.ok(provisionPage.includes("setCandidates(null);setCandidateError('');setMessage('')"),'new sessions must not retain an old candidate')
assert.ok(provisionPage.includes('if(epoch === generation.current)setMessage(error.message)'),'old operations must not publish errors on a new form')
