...
const independentRekey=systemSettingsView({},notificationConfig,{}, {revision:11,defaults:{rekey_minutes:0,ike_rekey_minutes:0}})
assert.equal(independentRekey.__ike_rekey_supported,true)
const ikeSaved=await systemAPI.saveRekeySettings({...independentRekey,rekey:{minutes:0,ikeMinutes:600}})
assert.deepEqual(rekeyWrite,{defaults:{rekey_minutes:0,ike_rekey_minutes:600},revision:11})
assert.equal(ikeSaved.__saved_ike_rekey_minutes,600)
assert.equal(ikeSaved.rekey.minutes,0)
await assert.rejects(systemAPI.saveRekeySettings({...independentRekey,rekey:{minutes:0,ikeMinutes:1441}}),/invalid_ike_rekey_default/)
