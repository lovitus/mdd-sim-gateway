...
assert.ok(page.includes("if (current()) await loadAll(label === 'Nickname' || label === 'Disable' || label === 'Soft delete' || label === 'Delete')"),'profile mutations use reported inventory')
...
assert.throws(()=>profileRequest('enable',{eid:'eid-a',profile:{iccid:'profile-a',profileState:'disabled',profileNickname:'[MDD-DELETED] test'}}),/euicc_profile_soft_deleted/)
