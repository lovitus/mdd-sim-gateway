...
const nativeMedia = fs.readFileSync(path.join(root, 'webui/src/browserMedia.js'), 'utf8')
...
assert.ok(mediaCanary.indexOf('MDD_ADMISSION(media_check)') < mediaCanary.indexOf('same => n,Answer()'))
...
assert.ok(nativeMedia.includes('class NativeBrowserCall'))
assert.ok(nativeMedia.includes('api.prepareBrowserOutbound(this.instanceId, this.destination)'))
assert.ok(nativeMedia.includes("type: 'browser.call.hangup'"))
assert.ok(nativeMedia.includes("type: 'browser.call.dtmf'"))
assert.ok(nativeMedia.includes('this.muted ? silence.slice(0) : frame'))
assert.ok(nativeMedia.includes('if (await this._stopIfEnding()) return'))
assert.ok(nativeMedia.includes('this.ending = true'))
assert.ok(nativeMedia.includes('revision <= this.lastCallRevision'))
