...
import { callOccupancy } from '../linePresentation.js'
...
export default function CallsV1({ instances, selected: selectedLine, setSelected, callAudioBufferMS, callCoordinator: coordinator, showToast }) {
  ...
  return <div className="u-page">
    ...
    <div className="card u-panel"><div className="u-card-head"><h2>{t('Call history')}</h2><select aria-label={t('History scope')} value={historyFilter} onChange={event => setHistoryFilter(event.target.value)}><option value="all">{t('All lines and transports')}</option><option value="selected">{t('Selected line and transport')}</option></select><div className="u-inline"><button className="btn btn-ghost" onClick={coordinator.loadHistory}>{t('Refresh')}</button><button className="btn btn-ghost" disabled={!visibleHistory.some(call => call.ended_at)} onClick={async () => { const ids = visibleHistory.filter(call => call.ended_at).map(call => call.id); if ((await dialogs.confirm(t('Delete all ended call records?')))) coordinator.deleteHistory(ids).catch(error => showToast(error.message)) }}>{t('Clear ended')}</button><button className="btn btn-danger-outline" disabled={!selectedHistory.size} onClick={async () => { const ids = [...selectedHistory]; if ((await dialogs.confirm(t('Delete selected call records?')))) coordinator.deleteHistory(ids).then(() => setSelectedHistory(new Set())).catch(error => showToast(error.message)) }}>{t('Delete selected')}</button></div></div>
