...
function SecretField({ label, value, onChange }) {
  ...
  return <div>
    <label>{label}</label>
    <input type="password" autoComplete="new-password" value={value.value}
...
export default function NotificationsV1({ showToast }) {
  ...
  const load = useCallback(() => api.notificationConfig().then(value => setDraft(editable(value))), [])
  useEffect(() => { void load(); void loadDeliveries() }, [load, loadDeliveries])
  ...
  const save = async () => {
    ...
    try {
      ...
      setDraft(editable(updated)); showToast(t('Saved'))
  ...
  const test = async channel => {
    ...
    try {
      ...
      showToast(`${t('Test queued')}: ${result.delivery?.state || 'pending'}`)
      ...
      <div className="u-device-grid">
        <div className="card u-panel"><div className="u-card-head"><h2>Webhook</h2><input type="checkbox" className="u-toggle"
          ...
          <div className="u-form-grid"><div><label>{t('Payload format')}</label><select value={draft.webhook.format}
            ...
            <div><label>{t('Method')}</label><select value={draft.webhook.method} onChange={event => patch('webhook', { method: event.target.value })}><option>POST</option><option>GET</option></select></div></div>
          ...
          <EventOptions draft={draft} channel="webhook" setDraft={setDraft}/><button className="btn btn-ghost" disabled={!!busy} onClick={() => test('webhook')}>{t('Test')}</button>
        </div>
        <div className="card u-panel"><div className="u-card-head"><h2>Telegram</h2><input type="checkbox" className="u-toggle"
          ...
          {draft.telegram.proxy_mode === 'country' && <><label>{t('Country exit')}</label><input maxLength="2" value={draft.telegram.proxy_country} onChange={event => patch('telegram', { proxy_country: event.target.value.toLowerCase() })}/></>}
          <EventOptions draft={draft} channel="telegram" setDraft={setDraft}/><button className="btn btn-ghost" disabled={!!busy} onClick={() => test('telegram')}>{t('Test')}</button>
        </div>
        <div className="card u-panel"><div className="u-card-head"><h2>PushPlus</h2><input type="checkbox" className="u-toggle"
          ...
          <EventOptions draft={draft} channel="pushplus" setDraft={setDraft}/><button className="btn btn-ghost" disabled={!!busy} onClick={() => test('pushplus')}>{t('Test')}</button>
        </div>
      </div>
