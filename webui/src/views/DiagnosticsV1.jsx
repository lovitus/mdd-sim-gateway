...
export default function DiagnosticsV1({ instances, devices, callCoordinator, showToast }) {
  ...
  return <div className="u-page"><div className="u-card-head"><div><h2>{t('Diagnostics')}</h2><p>{t('These are explicit read-only or no-charge tests. No automatic carrier calls or SMS are generated.')}</p></div><button className="btn btn-ghost" onClick={load}>{t('Refresh')}</button></div>
