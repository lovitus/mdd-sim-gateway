...
export function ImsCapabilityBadges({ device, compact = false }) {
  const { t, language } = useI18n()
  const isZh = language === 'zh'
  const vowifiActual = capability(device, 'vowifi')?.actual
  const isOnline = vowifiActual === 'on'

  return (
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', margin: compact ? '8px 0 2px 0' : '12px 0 6px 0', padding: compact ? '4px 8px' : '6px 10px', background: 'var(--bg-subtle, rgba(0,0,0,0.03))', borderRadius: 'var(--radius, 6px)', border: '1px solid var(--border-subtle, rgba(0,0,0,0.06))' }}>
      <span style={{ fontSize: 11, color: 'var(--text-mute)', fontWeight: 600, marginRight: 2 }}>{t('IMS Capabilities')}:</span>
      <span className={`badge ${isOnline ? 'badge-on' : 'badge-off'}`} style={{ fontSize: 10, padding: '2px 6px', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
        <span>📞</span> {isZh ? '语音 (Voice)' : 'Voice'} {isOnline ? '✓' : '—'}
      </span>
      <span className={`badge ${isOnline ? 'badge-on' : 'badge-off'}`} style={{ fontSize: 10, padding: '2px 6px', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
        <span>✉️</span> {isZh ? '短信 (SMS)' : 'SMS'} {isOnline ? '✓' : '—'}
      </span>
      <span className="badge badge-off" style={{ fontSize: 10, padding: '2px 6px', opacity: 0.6, display: 'inline-flex', alignItems: 'center', gap: 3 }}>
        <span>💬</span> RCS <span style={{ fontSize: 9 }}>({isZh ? '未启用' : 'N/A'})</span>
      </span>
    </div>
  )
}
    ...
    {pending ? <Discovering t={t} /> :
      ...
      <div className="u-device-grid">{devices.map((d, i) => <div className="card u-device-card" key={d.id}>
        ...
        <div className="u-card-body">{supportsCellular(d) && <CapabilitySwitch device={d} kind="cellular" compact onChanged={refreshDevices} showToast={showToast} />}<CapabilitySwitch device={d} kind="vowifi" compact onChanged={refreshDevices} showToast={showToast} onNavigateToHardware={() => { setSelectedDeviceId(d.id); setView('devices') }} /><LineActivity device={d} compact />{capability(d, 'vowifi').desired && <VowifiHistory instanceId={d.instance_id} subscribe={subscribe} compact />}
          ...
          <ImsCapabilityBadges device={d} compact />
