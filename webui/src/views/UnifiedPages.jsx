...
export function NotificationsPage({ showToast }) {
  ...
  useEffect(() => { api.notificationConfig().then(setS).catch(() => setS({ webhook: {}, telegram: {}, pushplus: {} })); loadDeliveries() }, [])
  ...
  const save = async () => { try { const saved = await api.saveNotificationConfig(s); setS(saved); showToast(t('Saved')) } catch (e) { showToast(e.message) } }
