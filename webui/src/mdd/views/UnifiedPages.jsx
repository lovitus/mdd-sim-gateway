...
export function CapabilitySwitch({ device, kind, onChanged, showToast, compact = false, onNavigateToHardware, onNavigateToSim }) {
  ...
  const unavailable = device.stale || (!c.available && !isImeiMissing) || c.actual === 'unsupported' || device.compatibilityOnly ||
    device.present === false || (kind === 'cellular' && capability(device, 'flight').desired)
  const title = kind === 'cellular' ? t('Allow data borrowing') : kind === 'connection' ? t('4G connection') : kind === 'flight' ? t('Flight mode') : kind === 'roaming' ? t('Allow data roaming') : t('VoWiFi / WiFi Calling')
    ...
    try {
      ...
      if (kind === 'vowifi' && (errorCode === 'sim_apdu_data_active' || errorCode === 'flight_mode_enabled')) {
        ...
        showToast?.(errorCode === 'flight_mode_enabled'
          ? t('VoWiFi intent was saved; turn off flight mode to prepare the SIM and start automatically')
...
function HardwarePanel({ device, refreshDevices, showToast }) {
  ...
  const loadPool = () => {
    api.imeiPool().then(data => {
      setPool(data?.pool || [])
      setPoolSnapshot(data)
    }).catch(error => { setPoolSnapshot(null); showToast(error.message) })
  }
  ...
  const save = async () => {
    ...
    try {
      await saveReaderIMEI(device, { id:selectedPoolId, imei: digits, name: name.trim() },poolSnapshot)
      await refreshDevices()
      loadPool()
      showToast(t('SIM presentation IMEI saved; physical modem identity is unchanged.'))
    } catch (error) {
      ...
      {isReader && (
        <div className="u-hardware-action u-hardware-imei">
          <div className="u-hardware-action-copy">
            <h4>{t('SIM presentation IMEI')}</h4>
            <p>{isZh ? '该 IMEI 绑定当前 SIM 卡，不修改物理设备标识。' : 'This IMEI is bound to the current SIM, not the physical reader or modem.'}</p>
          </div>
...
export function UnifiedOverview({
  ...
  const counts = useMemo(() => ({
    ...
    cellular: devices.filter(d => capability(d, 'connection').actual === 'on').length,
    ...
    {pending ? <Discovering t={t} /> :
      ...
      <div className="u-device-grid">{devices.map((d, i) => <div className="card u-device-card" key={d.id}>
        ...
        <div className="u-card-body">{supportsCellular(d) && <><CapabilitySwitch device={d} kind="connection" compact onChanged={refreshDevices} showToast={showToast} /><CapabilitySwitch device={d} kind="cellular" compact onChanged={refreshDevices} showToast={showToast} /><CapabilitySwitch device={d} kind="roaming" compact onChanged={refreshDevices} showToast={showToast} /></>}<CapabilitySwitch device={d} kind="vowifi" compact onChanged={refreshDevices} showToast={showToast} onNavigateToHardware={() => { setSelectedDeviceId(d.id); setDeviceTab('hardware'); setView('devices') }} onNavigateToSim={() => { setSelectedDeviceId(d.id); setDeviceTab('sim'); setView('devices') }} /><LineActivity device={d} compact /><BrowserVoiceStatus device={d} instances={instances} callCoordinator={callCoordinator} compact />{capability(d, 'vowifi').desired && <VowifiHistory instanceId={d.instance_id} subscribe={subscribe} compact />}
          ...
          {d.instance_id && <AllowancePanel instanceId={String(d.instance_id)} availableTransports={['vowifi','cellular'].filter(transport => instances.find(line => String(line.id) === String(d.instance_id))?.operations?.[`${transport}_sms`]?.ready === true)} showToast={showToast} />}
...
function RecycleBinPanel({ refresh, showToast }) {
  ...
  const load = useCallback(() => {
    ...
    api.softDeletedInstances()
      .then(res => { setItems(res.instances || []); setError('') })
  ...
  const restore = async (id) => {
    ...
    const item = items.find(value => String(value.id) === String(id))
    if (!item) return
    inFlight.current = true
    setRestoring(id)
    try {
      await api.restoreInstance(id,item.__catalog_revision)
      showToast(isZh ? `已恢复线路 ${id}，保持停用。` : `Line ${id} restored and remains disabled.`)
      await refresh?.()
      load()
    } catch (e) {
  ...
  if (error) return <div className="u-error" role="alert">{error}<button className="btn btn-ghost" onClick={load}>{t('Retry')}</button></div>
  ...
  return <div className="card u-panel">
    ...
    <p className="u-note">{isZh ? '线路归档保留配置和历史。恢复后保持停用，不会自动启动。' : 'Archived lines retain settings and history. Restored lines remain disabled and do not start automatically.'}</p>
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 14 }}>
      {items.map(item => (
        <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', border: '1px solid var(--border)', borderRadius: 10 }}>
          ...
          <button className="btn btn-primary" disabled={!!restoring} onClick={() => restore(item.id)}>
  ...
  return <div className="u-page">
    ...
    {!Object.keys(profiles).length ? <Empty title={t('No proxies configured')} detail={t('Add a subscription, individual node, or SOCKS5 proxy above.')} /> : <div className="u-proxy-list">{Object.entries(profiles).map(([id, profile]) => {
      ...
      return <div className="card u-proxy-row" key={id}>
        ...
        <div className="u-proxy-actions">{['node', 'socks5', 'cellular_sim'].includes(profile.type) && <><button className="btn btn-ghost" disabled={profileTests[id]?.busy} onClick={() => testProfile(id)}>{t(profileTests[id]?.busy ? 'Testing…' : profile.type === 'cellular_sim' ? 'Test borrowed data' : 'Test node UDP')}</button>{profileTests[id]?.ok && <small className="u-test-ok">{t('Passed')} · {profileTests[id].latency} ms</small>}{profileTests[id] && !profileTests[id].busy && !profileTests[id].ok && <small className="u-test-error" title={profileTests[id].error}>{profileTests[id].error || t('Failed')}</small>}</>}<button className="btn btn-ghost u-proxy-remove" onClick={() => removeProfile(id)}>{t('Remove')}</button></div>
      </div>
    })}</div>}
...
export function NotificationsPage({ showToast }) {
  ...
  const secretInput = (key, cfg, field, password = false, rows = 0) => {
    const props = {value:cfg[field] || '',disabled:cfg.__clear?.[field] === true,
      placeholder:cfg.__configured?.[field] ? t('Configured; leave blank to keep') : t('Not configured'),
      onChange:event => setChannel(key, {[field]:event.target.value})}
    return <div>{rows ? <textarea {...props} rows={rows}/> : <input {...props} type={password ? 'password' : 'text'} autoComplete="off"/>}
      <small>{cfg.__configured?.[field] ? t('Saved on server') : t('Not configured')}</small>
...
export function SystemPage({ showToast, openUpdateDialog, instances, callCoordinator, setSelected, setView }) {
  ...
  const loadStatus = () => api.systemRuntime().then(value => setStatus({...value,version:value.build_version || value.module_version || ''})).catch(error => showToast(error.message))
  useEffect(() => { api.settings().then(setS).catch(error => setSettingsError(error.message)); loadStatus() }, [])
  if (!s) return settingsError ? <p role="alert" className="u-error">{settingsError}<button className="btn btn-ghost" onClick={() => api.settings().then(value => {setS(value);setSettingsError('')}).catch(error => setSettingsError(error.message))}>{t('Retry')}</button></p> : <p>{t('Loading')}…</p>
  ...
  const save = () => saveDomain(tab)
  const saveRekey = async () => {
    if(settingsLock.current)return
    settingsLock.current=true;setSettingsBusy(true);setSettingsError('')
    try {setS(await api.saveRekeySettings(s));showToast(t('Rekey default saved; Apply is required.'))}
    catch(error){setSettingsError(error.message)}
    finally{settingsLock.current=false;setSettingsBusy(false)}
  }
  ...
  const action = async () => { try { await api.createBackup(); showToast(t('Backup download started')); loadStatus() } catch (e) { showToast(e.message) } }
  const checkUpdate=async()=>{if(settingsLock.current)return;settingsLock.current=true;setSettingsBusy(true);setChecking(true);try{const result=await api.checkUpdate(true);setUpdate(result);showToast(result.error_code || result.error || t(result.comparison_known === false ? 'Unknown' : result.update_available ? 'Update available' : 'Already up to date'))}catch(e){setUpdate(previous=>({...previous,ok:false,update_available:false,error_code:e.code||'update.error.unavailable'}));showToast(e.message)}finally{settingsLock.current=false;setSettingsBusy(false);setChecking(false)}}
  ...
  return <div className="u-page">{settingsError && <p role="alert" className="u-error">{settingsError}</p>}{(s.__load_errors || []).map(error => <p key={error.source} role="alert" className="u-error">{t(error.source)}: {error.code}</p>)}{s.__load_errors?.length > 0 && <button className="btn btn-ghost" disabled={settingsBusy} onClick={async () => {if(settingsLock.current)return;settingsLock.current=true;setSettingsBusy(true);try{setS(await api.settings());setSettingsError('')}catch(error){setSettingsError(error.message)}finally{settingsLock.current=false;setSettingsBusy(false)}}}>{t('Reload settings (discard edits)')}</button>}<div className="u-tabs">{tabs.map(([k, l]) => <button key={k} className={tab === k ? 'active' : ''} onClick={() => setTab(k)}>{l}</button>)}</div><div className="card u-panel">
    {tab === 'general' && <><h2>{t('General')}</h2><div className="u-form-grid"><div><label>{t('Language')}</label><select value={language} onChange={e => setLanguage(e.target.value)}><option value="zh">中文</option><option value="en">English</option></select></div><div><label>{t('Timezone')}</label><input disabled={settingsBusy || !s.__general_supported} list="timezones" value={s.timezone || ''} onChange={e => setS({ ...s, timezone: e.target.value })} /><datalist id="timezones"><option>Asia/Shanghai</option><option>Europe/London</option><option>America/New_York</option><option>America/Los_Angeles</option><option>Asia/Tokyo</option><option>UTC</option></datalist></div></div>
    <fieldset disabled={settingsBusy || !s.__device_defaults_supported}>
      {!s.__device_defaults_supported && <legend>{t('Not available in the current Go settings API')}</legend>}
      <h3>{t('New device defaults')}</h3>
      <label><input type="checkbox" className="u-toggle" checked={s.device_defaults?.cellular_enabled===true} onChange={e=>setS({...s,device_defaults:{...s.device_defaults,cellular_enabled:e.target.checked}})}/>{t('Enable 4G for newly detected modems')}</label>
      <label><input type="checkbox" className="u-toggle" checked={s.device_defaults?.vowifi_enabled===true} onChange={e=>setS({...s,device_defaults:{...s.device_defaults,vowifi_enabled:e.target.checked}})}/>{t('Enable VoWiFi for newly detected modems')}</label>
      <button className="btn btn-ghost" onClick={()=>saveDomain('device-defaults')}>{language==='zh'?'保存新设备默认值':'Save new device defaults'}</button>
    </fieldset>
    <fieldset disabled={settingsBusy || !s.__hardware_supported}>{!s.__hardware_supported && <legend>{s.__hardware_error || t('Not available in the current Go settings API')}</legend>}<h3>{t('Hardware')}</h3><label><input type="checkbox" className="u-toggle" ref={node => {if(node)node.indeterminate=!s.__hardware_supported}} aria-checked={s.__hardware_supported?s.hardware?.modem_backend==='serial':'mixed'} checked={s.hardware?.modem_backend==='serial'} onChange={e => {
      const serial = e.target.checked
      setS({ ...s, hardware: { ...s.hardware, modem_backend: serial ? 'serial' : 'auto' } })
    }} />{t('VoWiFi-only mode (do not run ModemManager)')}</label><p role="status" className="u-hint">{s.__hardware_detail?t(s.__hardware_detail):s.__hardware_runtime==='not_observed'?(language==='zh'?'运行模式尚未确认':'Runtime mode not confirmed'):s.__hardware_runtime}</p><button className="btn btn-ghost" onClick={()=>saveDomain('hardware')}>{language==='zh'?'保存并切换宿主模式':'Save and switch host mode'}</button></fieldset></>}
    {tab === 'web' && <fieldset disabled={settingsBusy || !s.__web_supported}>{!s.__web_supported && <legend>{t('Startup configuration; editing is not available')}</legend>}{s.__web_restart_required && <p role="status" className="u-note">{t('Web access changes require a Core restart.')}</p>}<h2>{t('Web access')}</h2><label><input type="checkbox" disabled className="u-toggle" ref={node => {if(node)node.indeterminate=typeof s.tls?.self_signed!=='boolean'}} aria-checked={typeof s.tls?.self_signed==='boolean'?s.tls.self_signed:'mixed'} checked={s.tls?.self_signed===true} onChange={e => setS({ ...s, tls: { ...s.tls, self_signed: e.target.checked } })} />{t('Use self-signed certificate')}</label><div className="u-form-grid"><div><label>{t('Bind address')}</label><input value={s.bind || ''} onChange={e => setS({ ...s, bind: e.target.value })} /></div><div><label>{t('HTTPS port')}</label><input type="number" value={s.http_port ?? ''} onChange={e => setS({ ...s, http_port: +e.target.value })} /></div><div><label>{t('Domain')}</label><input disabled value={s.tls?.domain || ''} onChange={e => setS({ ...s, tls: { ...s.tls, domain: e.target.value } })} /></div><div><label>{t('Certificate path')}</label><input value={s.tls?.cert_path || ''} onChange={e => setS({ ...s, tls: { ...s.tls, cert_path: e.target.value } })} /></div><div><label>{t('Private key path')}</label><input value={s.tls?.key_path || ''} onChange={e => setS({ ...s, tls: { ...s.tls, key_path: e.target.value } })} /></div></div></fieldset>}
    ...
    </div><div className="u-form-grid"><div><label>{t('CHILD rekey minutes (0 disables)')}</label><input type="number" min="0" max="1440" disabled={settingsBusy || !s.__rekey_supported} value={s.rekey?.minutes ?? ''} onChange={event=>setS({...s,rekey:{...s.rekey,minutes:Number(event.target.value)}})}/></div><div><label>{t("IKE rekey minutes (0 disables)")}</label><input type="number" min="0" max="1440" disabled={settingsBusy || !s.__ike_rekey_supported} value={s.rekey?.ikeMinutes ?? ""} onChange={event=>setS({...s,rekey:{...s.rekey,ikeMinutes:Number(event.target.value)}})}/></div></div><div className="u-inline"><button className="btn btn-ghost" disabled={settingsBusy || !s.__rekey_supported} onClick={saveRekey}>{t('Save rekey default')}</button><button className="btn btn-ghost" disabled={settingsBusy || !s.__rekey_supported || s.rekey?.minutes !== s.__saved_rekey_minutes || (s.__ike_rekey_supported && s.rekey?.ikeMinutes !== s.__saved_ike_rekey_minutes)} onClick={applyRekey}>{t('Apply saved catalog')}</button></div></>}
    ...
    {tab === 'security' && <>
      <h2>{t('Security')}</h2>
      <div className="u-detail"><span>{t('HTTPS')}</span><b>{status?.public?.transport ? t(status.public.transport === 'https+wss' ? 'Enabled' : 'Disabled') : t('Unknown')}</b></div>
      <div className="u-detail"><span>{t('Certificate mode')}</span><b>{typeof status?.public?.certificate?.self_signed==='boolean' ? t(status.public.certificate.self_signed?'Self-signed':'Non-self-signed') : '—'}</b></div>
      <div className="u-detail"><span>{t('Certificate expiry')}</span><b>{Number.isFinite(Date.parse(status?.public?.certificate?.not_after)) ? new Date(status.public.certificate.not_after).toLocaleString() : '—'}</b></div>
    ...
    {tab === 'backup' && <>
      ...
      <fieldset disabled={settingsBusy || !s.__updates_supported}>{!s.__updates_supported && <legend>{t('Not available in the current Go settings API')}</legend>}<h3>{t('Update connection')}</h3>
      ...
      <div className="u-form-grid">
        <div><label>{t('Connection')}</label><select value={s.updates?.proxy_mode || ''} onChange={e => setS({ ...s, updates: { proxy_mode: e.target.value, proxy_profile_id: e.target.value === 'library' ? (s.updates?.proxy_profile_id || '') : '' } })}><option value="" disabled>—</option><option value="auto">{t('Automatic — direct, then proxy library')}</option><option value="direct">{t('Direct only')}</option><option value="library">{t('Specified proxy')}</option></select></div>
        {s.updates?.proxy_mode === 'library' && <div><label>{t('Proxy')}</label><select value={s.updates?.proxy_profile_id || ''} onChange={e => setS({ ...s, updates: { proxy_mode: 'library', proxy_profile_id: e.target.value } })}><option value="">{t('Select a proxy…')}</option>{Object.entries(s.proxy?.profiles || {}).map(([id, profile]) => <option key={id} value={id}>{profile.name || t('Unnamed proxy')}{profile.unavailable ? ` (${t('Unavailable')})` : ''}</option>)}</select></div>}
      </div>
      ...
      <div className="u-detail"><span>{t('Software updates')}</span><div className="u-inline"><b>{update?.latest ? `v${update.latest}` : t(update ? 'Update check failed' : 'Not checked')}</b><button className="btn btn-ghost" disabled={checking || settingsBusy || (s.__updates_supported && (s.updates?.proxy_mode!==s.__saved_updates?.proxy_mode || (s.updates?.proxy_profile_id || '')!==(s.__saved_updates?.proxy_profile_id || '')))} onClick={checkUpdate}>{t(checking?'Checking…':'Check for updates')}</button></div></div>
      <button className="btn btn-ghost" onClick={() => openUpdateDialog(update || {latest:'',comparison_known:false,update_available:false})}>{t('Update status')}</button>
      ...
      {update?.ok&&update.network&&<div className="u-detail"><span>{t('Verified update route')}</span><b>{update.network.proxy_mode==='direct'?t('Direct only'):s.proxy?.profiles?.[update.network.proxy_profile_id]?.name||update.network.proxy_profile_id}</b></div>}
  ...
  </div>{['general','voice','security','backup','web'].includes(tab) && <button className="btn btn-primary" disabled={settingsBusy || (tab === 'web' ? !s.__web_supported : tab === 'general' ? !s.__general_supported : tab === 'security' ? !s.__security_supported : tab === 'backup' ? !s.__updates_supported : !s.__voice_supported)} onClick={save}>{t(tab === 'web' ? 'Save Web access settings' : tab === 'voice' ? 'Save call and VoWiFi settings' : tab === 'security' ? 'Save audit settings' : tab === 'backup' ? 'Save update networking' : 'Save')}</button>}</div>
}
...
function HostPanel({ host, alerts, loading, clearing, onClear, t }) {
  ...
  return <div className="u-device-grid">
    ...
    <div className="card u-panel">
      <h3>{t('Machine')}</h3>
      <Row label={t('CPU model')}>{host.model || '—'}</Row>
...
export function DiagnosticsPage(props) {
  ...
  const loadAgents = useCallback(async () => {
    if(agentReadBusy.current)return
    agentReadBusy.current=true;setAgentsLoading(true)
    try {const value=await api.agentHealth();if(mounted.current){setAgents(value.agents || []);setAgentsError('')}}
    catch(error){if(mounted.current)setAgentsError(error.message)}
    finally{agentReadBusy.current=false;if(mounted.current)setAgentsLoading(false)}
  },[])
  const loadHost = useCallback(async () => {
    if(hostReadBusy.current)return
    hostReadBusy.current=true;setHostLoading(true)
    try {const value=await api.diagnosticSystemStatus();if(mounted.current){setSystem(value);props.setSystemMeta?.(value);setHostError('')}}
    catch(error){if(mounted.current)setHostError(error.message)}
    finally{hostReadBusy.current=false;if(mounted.current)setHostLoading(false)}
  },[props.setSystemMeta])
