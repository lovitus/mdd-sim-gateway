...
export function DiagnosticsPage(props) {
  ...
  useEffect(() => {
    const load = () => api.diagnosticSystemStatus().then(value => { setSystem(value); props.setSystemMeta?.(value) }).catch(error => props.showToast(error.message)).finally(() => setHostLoading(false))
