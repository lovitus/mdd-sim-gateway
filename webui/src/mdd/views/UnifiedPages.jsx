...
export function SystemPage({ showToast, openUpdateDialog, instances, callCoordinator, setSelected, setView }) {
  ...
  const loadStatus = () => api.systemStatus().then(setStatus).catch(error => showToast(error.message))
  ...
  return <div className="u-page">{settingsError && <p role="alert" className="u-error">{settingsError}</p>}<div className="u-tabs">{tabs.map(([k, l]) => <button key={k} className={tab === k ? 'active' : ''} onClick={() => setTab(k)}>{l}</button>)}</div><div className="card u-panel">
    ...
    </div></fieldset><div className="u-form-grid"><div><label htmlFor="cellular-audio-buffer-ms">{t('Call audio buffer limit (ms)')}</label><input id="cellular-audio-buffer-ms" disabled={settingsBusy} type="number" min="100" max="2000" step="1" value={s.cellular_audio_buffer_ms ?? 500} onChange={e => setS({ ...s, cellular_audio_buffer_ms: +e.target.value })} /><p className="u-hint">{t('Call audio buffer hint')}</p></div>
  ...
  </div>{['general','voice'].includes(tab) && <button className="btn btn-primary" disabled={settingsBusy} onClick={save}>{t(tab === 'voice' ? 'Save audio and call timeout' : 'Save')}</button>}</div>
}
