...
function ReplayNotificationModal({ target, notification, busy, onClose, onConfirm }) {
  const { t, language } = useI18n()
  const [understood, setUnderstood] = useState(false)
  if (!notification) return null
  const isZh = language === 'zh'
  const seq = notification.seqNumber ?? notification.seq
  const op = notification.profileManagementOperation || notification.operation || 'delete'
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#0008', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60 }}
      onClick={onClose}>
      <div className="card" style={{ width: 440, maxWidth: '92vw', padding: 22, border: '1px solid #e11d48' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#e11d48', fontWeight: 700, fontSize: 16, marginBottom: 8 }}>
          <span>⚠️</span>
          <span>{isZh ? '重放删除/管理通知 (高风险操作)' : 'Replay Notification (High Risk)'}</span>
        </div>
        <div style={{ fontSize: 13, lineHeight: 1.5, color: 'var(--text-main)', marginBottom: 12 }}>
          {isZh ? (
            <>
              <p style={{ margin: '0 0 8px 0' }}>
                向运营商服务器 (SM-DP+/SM-DS) <strong>重放删除通知</strong>将正式确认此 eSIM Profile 已被移除。
              </p>
              <p style={{ margin: '0 0 8px 0', color: '#e11d48', fontWeight: 600 }}>
                警告：在网络中断或换机解绑失败时，这是释放名额的唯一手段。但一旦重放成功，运营商可能永久废弃并注销该卡，导致其无法再次使用！
              </p>
            </>
          ) : (
            <>
              <p style={{ margin: '0 0 8px 0' }}>
                Replaying this notification to SM-DP+/SM-DS will formally confirm to the carrier that this profile has been removed.
              </p>
              <p style={{ margin: '0 0 8px 0', color: '#e11d48', fontWeight: 600 }}>
                Warning: This is the definitive way to unbind in extreme network failure, but it may cause the carrier to permanently decommission this eSIM!
              </p>
            </>
          )}
        </div>
        <div style={{ background: 'var(--bg-subtle, #f4f4f5)', padding: '10px 12px', borderRadius: 8, fontSize: 12, marginBottom: 14, fontFamily: 'monospace' }}>
          <div>Seq: #{seq}</div>
          <div>Op: {op}</div>
          <div>ICCID: {notification.iccid || '—'}</div>
        </div>
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, marginBottom: 16, cursor: 'pointer' }}>
          <input type="checkbox" checked={understood} onChange={(e) => setUnderstood(e.target.checked)} />
          <span style={{ fontWeight: 600 }}>{isZh ? '我已知晓风险并确认重放通知' : 'I understand the risks and confirm replay'}</span>
        </label>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button className="btn btn-ghost" onClick={onClose} disabled={busy}>{t('Cancel')}</button>
          <button className="btn btn-danger" disabled={busy || !understood} onClick={onConfirm} style={{ background: '#e11d48', color: '#fff' }}>
            {busy ? (isZh ? '重放中…' : 'Replaying…') : (isZh ? '确认重放' : 'Confirm Replay')}
          </button>
        </div>
      </div>
    </div>
...
function seTarget(reader, se, profile) {
  return {
    reader,
    eid:se?.eid,
    profile,
    notifications:se?.notifications || [],
    se_id: se?.id || se?.seId,
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#0008', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}
      ...
      <div className="card" role="dialog" aria-modal="true" aria-labelledby="esim-download-title" style={{ width: 480, maxWidth: '92vw', maxHeight: 'calc(100dvh - 32px)', overflowY: 'auto', padding: 20 }} onClick={(e) => e.stopPropagation()}
        ...
        <label style={{ display: 'block', marginBottom: 10 }}>
          ...
          <input type="password" autoComplete="off" value={confirmation} onChange={(e) => setConfirmation(e.target.value)} style={{ width: '100%' }} />
        </label>
        <label style={{ display: 'block', marginBottom: 10 }}>
...
function isLineRunning(inst) {
  return inst?.operations?.vowifi_call?.ready === true || inst?.operations?.vowifi_sms?.ready === true
}
...
export default function Esim({ cards, instances, refresh, subscribe, showToast }) {
  ...
  }, [reader, selectedCard?.iccid, selectedCard?.session_generation])
  ...
  const requestLoad = useCallback(async () => {
    ...
    await loadAll()
  }, [reader, loading, busyOp, loadAll])
  const requestDownload = useCallback(() => {
    if (reader && !busyOp) setShowDl(true)
  }, [reader, busyOp])
  useEffect(() => {
    const receipt = receiptRef.current
    if (!receipt || downloadView(receipt.job).terminal) return
    let stopped = false, timer, delay = 30000
    const deadline = Date.now() + 600000
    const check = async () => {
      if (stopped || downloadView(receiptRef.current?.job).terminal) return
      try {
        const result = await api.euiccDownloadStatus(receipt.eid, receipt.operation_id)
        if (stopped) return
        applyDownload({...receipt,job:result.job})
        if (downloadView(result.job).terminal) {
          if (result.job.state === 'completed') { await loadAll(); await refresh?.() }
          return
        }
      } catch (error) {
        if (!stopped) setErr(error.message)
      }
      if (!stopped && Date.now() + delay < deadline) {
        delay = Math.min(240000, delay * 2)
        timer = setTimeout(check, Math.min(delay, deadline - Date.now()))
      } else if (!stopped) setErr(t('Download observation timed out; the operation was not restarted.'))
    }
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
            <div>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-dim)' }}>
                {t('Reading does not stop this line. Conflicting eSIM operations may be rejected while it is active.')}
        ...
        {!hasEuicc ? (
          <div style={{ color: 'var(--text-mute)', fontSize: 13, lineHeight: 1.5 }}>
            {status && !status.available
              ? t('No eUICC inventory available.')
              : loading
                ? t('Reading…')
                : !loaded
                  ...
                  {!seProfiles.length ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {seProfiles.map((p) => {
                        ...
                        const target = seTarget(reader, se, p)
                      ...
                      return (
                        <div key={`${se.id}:${n.seqNumber ?? n.seq}`} style={{
                          ...
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button className="btn btn-ghost" style={{ color: '#e11d48', borderColor: '#fca5a5' }}
                              disabled={true}
                              title={t('Replay deletion/management notification to SM-DP+/SM-DS')}
                              onClick={() => setReplayTarget({ se, notification: n })}>
                              {t('Replay')}
                            </button>
      ...
      {renameTarget && (
        <RenameModal
          ...
          onSave={(nick) => {
            ...
            const target = seTarget(reader, se, profile)
            runProfileOp('Nickname', () => api.esimNickname(profile.iccid, nick, target)
              .then(result => { setRenameTarget(null); return result }))
          }}
        />
      )}

      {replayTarget && (
        <ReplayNotificationModal
          target={seTarget(reader, replayTarget.se)}
          notification={replayTarget.notification}
          busy={busyOp === 'Replay'}
          onClose={() => setReplayTarget(null)}
          onConfirm={() => {
            const { se, notification } = replayTarget
            const target = {
              ...seTarget(reader, se),
              seq: notification.seqNumber ?? notification.seq,
              iccid: notification.iccid,
              confirmed: true,
            }
            runProfileOp('Replay', async () => {
              await api.esimReplayNotification(target)
              setReplayTarget(null)
              showToast?.(t('Notification replayed successfully'))
            })
          }}
        />
      )}
