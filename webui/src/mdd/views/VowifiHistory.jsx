...
// The original MDD chart, with the repository's already-adapted Go request lifecycle.
...
const REASON_LABEL = {
  ...
  reg_temporary: 'Server P-CSCF temporarily rejected IMS registration; retry is scheduled',
...
const EVIDENCE_LABEL = {
  ...
  server_pcscf_sip_temporary: 'Server P-CSCF {peer} returned temporary SIP {status}; retry in {retry_after}s',
...
export default function VowifiHistory({ instanceId, compact = false }) {
  ...
  useEffect(() => {
    let stopped = false, timer, delay = REFRESH_MS
    setData(null); setHover(null); setError('')
    const load = async () => {
      if (!instanceId || stopped) return
      try {
        const result = await api.lineAvailability(instanceId)
        if (!stopped) { setData(previous => JSON.stringify(previous) === JSON.stringify(result) ? previous : result); setError(''); delay = REFRESH_MS }
      } catch (err) {
        if (!stopped) { setError(err.message); delay = Math.min(delay * 2, 240000) }
      } finally {
  ...
  if (error && !data) return <div className="u-note u-error">{`${t('Error')}: ${error}`}</div>
  ...
  const legend = compact ? (present.length ? present : ['unknown']) : STATES
  ...
  return <div className={`u-uptime ${compact ? 'compact' : ''}`}>
    ...
    <div className="u-uptime-head">
      <div>
        ...
        <p>{compact
          ...
          : t('Past {window}', { window: duration(span, t) })}</p>
      </div>
    ...
    <div className="u-uptime-plot">
      ...
      {hover && <div className="u-uptime-tip" style={{ left: `clamp(0px, calc(${hover.center}% - 120px), max(0px, calc(100% - 240px)))` }}>
