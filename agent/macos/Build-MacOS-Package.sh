...
if [ "$SKIP_PYINSTALLER" -ne 1 ]; then
  PYINSTALLER_TARGET_ARGS=()
  if [ "$ARCHITECTURE" = "macos-arm64" ]; then
    PYINSTALLER_TARGET_ARGS+=(--target-arch arm64)
  fi
  (cd "$AGENT_DIR" && "$PYTHON" -m PyInstaller --noconfirm --clean \
    --distpath "$DIST_DIR" --workpath "$PYINSTALLER_WORKPATH" \
    "${PYINSTALLER_TARGET_ARGS[@]}" mdd-agent.spec)
  (cd "$AGENT_DIR" && "$PYTHON" -m PyInstaller --noconfirm --clean \
    --distpath "$DIST_DIR" --workpath "$PYINSTALLER_WORKPATH" \
    "${PYINSTALLER_TARGET_ARGS[@]}" mdd-agent-gui.spec)
fi
