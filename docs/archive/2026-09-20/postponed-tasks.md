...
- 2026-09-17 09:04 UTC已补齐本批真实Chrome页面验收：同一正常浏览器入口恢复可用，
  未修改安全策略或绕过校验。EC20设备状态/4G与APN/硬件、通话及短信选择器、诊断健康/
  Agent主机/网关主机均实际点击核对；4G已连接、号码及能力一致、Linux版本和隔离状态正确。
  浏览器错误日志为空；未改开关、拨号、发送短信或重做故障注入，标签页和preview已关闭。
  历史浏览器工具阻塞已解除，不再是未完成项；不扩大为所有业务/长期稳定性无条件验收。
